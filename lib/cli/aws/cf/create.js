  ...
  program
    ...
    .action((stackName, programOptions) => {
      if(validateStack(stackName, options.LOCALLY_DEFINED_STACKS)) {
        ...
        if(programOptions.disableRollback) params.DisableRollback = true
        if(programOptions.parameter) params.Parameters = programOptions.parameter
...
function parameters(val, params) {
  if(! /\w+\=\w+/.test(val)) throw new Error(`Incorrect parameter format for '-p ${val}'. Should be '-p name=value'.`)
